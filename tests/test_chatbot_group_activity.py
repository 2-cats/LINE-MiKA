...
#normal case + no participation
class LeaveGroupActivityMessageTestCase2(unittest.TestCase):
    def setUp(self):
        self.app = create_app('testing')
        self.app_context = self.app.app_context()
        self.app_context.push()
        ...
        db.session.add_all([activity_log])
        db.session.commit()
        print(leave_group_activity_message(activity.id, user.line_user_id))
        self.result_message = leave_group_activity_message(activity.id, user.line_user_id)
        self.expected_message = {
            "type": "text",
    ...
    def test_leave_group_activity_message222222(self):
